...
test_that("cryptfile is readable by decrypt", {
  ...
  writeBin(dat, cryptfile(file(tmp), "my secret"))
  # readBin(tmp, raw(), 100)
...
test_that("encrypt output is readable by cryptfile", {
  ...
  tst <- readBin(cryptfile(file(tmp), "my secret"), raw(), 100)
  expect_identical(tst, dat)
})
...
test_that("cryptfile works", {
  tmp <- tempfile()
  ref <- as.raw(1:255)
  writeBin(ref,  cryptfile(file(tmp), key = "my secret", verbosity = 0))
  tst <- readBin(cryptfile(file(tmp), key = "my secret", verbosity = 0),  raw(), 1000)
  expect_identical(tst, ref)

  tmp <- tempfile()
  ref <- deparse1(c('hello', 'there'))
  writeLines(ref,  cryptfile(file(tmp), key = "my secret", verbosity = 0))
  tst <- readLines(cryptfile(file(tmp), key = "my secret", verbosity = 0))
  expect_identical(tst, ref)
})
...
test_that("cryptfile works at multiple sizes", {
  ...
  for (N in 10^(2:7)) {
    ...
    writeBin(ref,  cryptfile(file(tmp), key = key, verbosity = 0))
    tst <- readBin(cryptfile(file(tmp), key = key, verbosity = 0),  raw(), N)
...
test_that("cryptfile doubler works at multiple sizes", {
  ...
  for (N in 10^(2:6)) {
    # message(N)
    ref <- as.raw(seq_len(N) %% 256)
    stream <- cryptfile(file(tmp), key = key, verbosity = 0)
    open(stream, 'wb')
    writeBin(ref, stream)
    writeBin(ref, stream)
    close(stream)
    ...
    if (TRUE) {
      tst <- readBin(cryptfile(file(tmp), key = key, verbosity = 0),  raw(), N * 4)
...
test_that("cryptfile text works at multiple sizes", {
  ...
  for (N in 10^c(1,3,5)) {
    ...
    # Stream connection reader
    tst <- readLines(cryptfile(file(tmp), key = key, verbosity = 0),  N)
...
test_that("cryptfile serialize works", {
  ...
  key <- argon2('my secret', rcrypto(16))
  dat <- mtcars
  saveRDS(dat, cryptfile(file(tmp), key))
  tst <- readRDS(cryptfile(file(tmp), key))
  expect_identical(tst, dat)
